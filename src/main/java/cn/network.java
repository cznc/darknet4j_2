...
import cn.wrapper.network_variables;

public class network extends network_variables implements Cloneable
{
...
{
	Load_args args =new Load_args();//{0};
    args.w = net.w;
    args.h = net.h;
    args.size = net.w;
...
public static network load_network(String cfg, String weights, boolean clear) throws IOException, Exception
{
    network net = parse_network_cfg(cfg);
    if(weights!=null && weights.length() != 0){
        load_weights(net, weights);
    }
    if(clear) (net.seen[0]) = 0;
    return net;
}

public static /*size_t*/int get_current_batch(network net)
{
    /*size_t*/int batch_num = (net.seen[0])/(net.batch*net.subdivisions);
    return batch_num;
}
...
{
    /*size_t*/int batch_num = get_current_batch(net);
    int i;
    float rate;
    ...
    switch (net.policy) {
        ...
        case SIG:
            return net.learning_rate * (1.f/(1.f+exp(net.gamma*(batch_num - net.step))));
        default:
...
String get_layer_string(LAYER_TYPE a)
{
    switch(a){
        case CONVOLUTIONAL:
...
{
    network net = new network();//calloc(1, sizeof(network));
    net.n = n;
    net.layers = new layer[net.n];//calloc(net.n, sizeof(layer));
    net.seen = callocInt(1, 1/*sizeof(size_t)*/);
    net.t    = callocInt(1, 1/*sizeof(int)*/);
    net.cost = calloc(1, 1/*sizeof(float)*/);
    return net;
}

static void forward_network(network netp)
{
//#ifdef GPU
//    if(netp->gpu_index >= 0){
...
//#endif
//    network net = *netp;
    network net=null;//new network();
//    System.arraycopy(netp, 0, net, 0, 1);
	try {
		net = (network)netp.clone();
...
static void update_network(network netp)
{
//#ifdef GPU
//    if(netp->gpu_index >= 0){
...
//    network net = *netp;
    ...
    int i;
    Update_args a = new Update_args();//{0};
...
static void calc_network_cost(network netp)
{
//    network net = *netp;
	network net=null;
	try {
...
static void backward_network(network netp)
{
//#ifdef GPU
//    if(netp->gpu_index >= 0){
...
static float train_network_datum(network net)
{
    net.seen[0] += net.batch;
    net.train = true/*1*/;
    forward_network(net);
    backward_network(net);
    float error = net.cost[0];
...
//        else if(l.type == LAYER_TYPE.CROP){
//            resize_crop_layer(l, w, h);
//        }
        else if(l.type == LAYER_TYPE.MAXPOOL){
            resize_maxpool_layer(l, w, h);
        }else if(l.type == LAYER_TYPE.REGION){
            resize_region_layer(l, w, h);
        }
//        else if(l.type == LAYER_TYPE.ROUTE){
//            resize_route_layer(l, net);
//        }else if(l.type == LAYER_TYPE.REORG){
//            resize_reorg_layer(l, w, h);
//        }else if(l.type == LAYER_TYPE.AVGPOOL){
//            resize_avgpool_layer(l, w, h);
//        }else if(l.type == LAYER_TYPE.NORMALIZATION){
//            resize_normalization_layer(l, w, h);
//        }else if(l.type == LAYER_TYPE.COST){
...
//        }
        ...
        if(l.workspace_size > 2000000000) assert(false/*0*/);
        inputs = l.outputs;
        net.layers[i] = l;
        w = l.out_w;
        h = l.out_h;
        if(l.type == LAYER_TYPE.AVGPOOL) break;
    }
...
//    int i;
//    for(i = 0; i < net.n; ++i){
//        if(net.layers[i].type == LAYER_TYPE.DETECTION){
//            return net.layers[i];
//        }
//    }
//    fprintf(stderr, "Detection layer not found!!\n");
//    layer l = new detection_layer();//{0};
//    return l;
//}
...
//    float[] X = calloc(net.batch*test.X.rows, 1/*sizeof(float)*/);
//    for(i = 0; i < test.X.rows; i += net.batch){
//        for(b = 0; b < net.batch; ++b){
//            if(i+b == test.X.rows) break;
//            memcpy(X+b*test.X.cols, test.X.vals[i+b], test.X.cols*1/*sizeof(float)*/);
//        }
//        for(m = 0; m < n; ++m){
...
//    int k = net.outputs;
//    matrix pred = make_matrix(test.X.rows, k);
//    float []X = calloc(net.batch*test.X.cols, 1/*sizeof(float)*/);
//    for(i = 0; i < test.X.rows; i += net.batch){
//        for(b = 0; b < net.batch; ++b){
//            if(i+b == test.X.rows) break;
//            memcpy(X+b*test.X.cols, test.X.vals[i+b], test.X.cols*1/*sizeof(float)*/);
...
public static layer get_network_output_layer(network net)
{
    int i;
    for(i = net.n - 1; i >= 0; --i){
        if(net.layers[i].type != LAYER_TYPE.COST) break;
    }
    return net.layers[i];
}
...
//    float[] errors = (float[] ) calloc(n, 1/*sizeof(float)*/);
//
//    float sum = 0;
//    for(i = 0; i < n; ++i){
