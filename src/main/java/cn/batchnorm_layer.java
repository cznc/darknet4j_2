...
import cn.wrapper.batchnorm_layer_wrapper;

public class batchnorm_layer extends batchnorm_layer_wrapper{

@Override
public void forward(layer l, network network) {
	// TODO Auto-generated method stub
	forward_batchnorm_layer(l, network);
}

@Override
public void backward(layer l, network network) {
	// TODO Auto-generated method stub
	backward_batchnorm_layer(l, network);
}
...
layer make_batchnorm_layer(int batch, int w, int h, int c)
{
    ...
    layer l = new batchnorm_layer();//{0};
    l.type = LAYER_TYPE.BATCHNORM;
    l.batch = batch;
    l.h = l.out_h = h;
    l.w = l.out_w = w;
    l.c = l.out_c = c;
    l.output = calloc(h * w * c * batch, 1/*sizeof(float)*/);
    l.delta  = calloc(h * w * c * batch, 1/*sizeof(float)*/);
    l.inputs = w*h*c;
    l.outputs = l.inputs;

    l.scales = calloc(c, 1/*sizeof(float)*/);
    l.scale_updates = calloc(c, 1/*sizeof(float)*/);
    l.biases = calloc(c, 1/*sizeof(float)*/);
    l.bias_updates = calloc(c, 1/*sizeof(float)*/);
    int i;
    for(i = 0; i < c; ++i){
        l.scales[i] = 1;
    }

    l.mean = calloc(c, 1/*sizeof(float)*/);
    l.variance = calloc(c, 1/*sizeof(float)*/);

    l.rolling_mean = calloc(c, 1/*sizeof(float)*/);
    l.rolling_variance = calloc(c, 1/*sizeof(float)*/);
...
{
    if(l.type == LAYER_TYPE.BATCHNORM) copy_cpu(l.outputs*l.batch, net.input, 1, l.output, 1);
...
public static void backward_batchnorm_layer(layer l, network net)
{
    ...
    if(l.type == LAYER_TYPE.BATCHNORM) copy_cpu(l.outputs*l.batch, l.delta, 1, net.delta, 1);
}
